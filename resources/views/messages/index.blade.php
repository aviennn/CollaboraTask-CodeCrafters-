...
<script>
    ...
    .listen('MessageSent', (e) => {
        console.log('New message received:', e); // Log the event object to confirm structure

        // Check if the `user` and other required properties are present
        if (e && e.user && e.message) {
            appendMessageToChat(teamId, {
                message_body: e.message, // Corrected mapping
                sender_name: e.user.name, // Directly map from `e.user`
                sender_avatar: e.user.profile_photo_path || null // Use a fallback if `profile_photo_path` is null
            }, e.user_id === {{ Auth::id() }}); // Check for sender identification
        } else {
            console.error('User object or message property is missing in the event:', e);
        }
    });
            });
        });
