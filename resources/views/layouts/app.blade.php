...
@media (min-width: 992px) {
 ...
 /* Light mode styling for DataTable labels */
...
body.dark-mode .dataTables_wrapper .dataTables_paginate .paginate_button {
    color: #e0e0e0; /* Light font color for visibility in dark mode */
}

/* Dark mode styling for input fields in DataTables */
body.dark-mode .dataTables_wrapper .dataTables_filter input,
body.dark-mode .dataTables_wrapper .dataTables_length select {
    background-color: #444; /* Dark background for inputs */
    color: #e0e0e0; /* Light font color in inputs */
    border: 1px solid #666; /* Border for better visibility */
}
   /* Sidebar link hover effect with dynamic color */
    .main-sidebar a:hover {
        background-color: var(--hover-bg-color, #e0e0e0); /* Fallback color if custom property not set */
    }
    </style>
...
<body class="hold-transition sidebar-mini">
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        function activateDarkMode() {
            ...
            adjustIconAndLinkColors(); // Adjust colors when dark mode is activated
        }
        ...
        function deactivateDarkMode() {
            ...
            adjustIconAndLinkColors(); // Adjust colors when dark mode is deactivated
        }

        // Function to calculate brightness from hex color
        ...
        // Function to dynamically adjust icon, link, and hover colors based on background brightness
        function adjustIconAndLinkColors() {
            ...
            // Determine text and hover color based on brightness
            function adjustColorBasedOnBrightness(bgColor, elements, hoverElements) {
                const brightness = getBrightness(rgbToHex(bgColor));
                const textColor = brightness < 128 ? '#ffffff' : '#000000';
                const hoverBackgroundColor = brightness < 128 ? '#444444' : '#e0e0e0';
                ...
                hoverElements.forEach(element => {
                    element.style.setProperty('--hover-bg-color', hoverBackgroundColor);
                });
            }

            // Apply color adjustments
            adjustColorBasedOnBrightness(navbarColor, links, []);
            adjustColorBasedOnBrightness(sidebarColor, links, links); // Apply hover color to sidebar links
            adjustColorBasedOnBrightness(navbarColor, icons, []);
            adjustColorBasedOnBrightness(sidebarColor, icons, icons); // Apply hover color to sidebar icons
        }

        // Initial call to adjust colors on page load
        adjustIconAndLinkColors();

        // Event listener to adjust colors when theme colors are changed
