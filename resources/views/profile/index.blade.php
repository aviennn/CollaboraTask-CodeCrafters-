         ...
         alt="Profile Picture">
                            ...
                            <div class="user-xp-level mt-3">
                                ...
                                <p>XP Until Next Level: {{ auth()->user()->xpUntilNextLevel() }} XP</p>
                                                    ...
                                                    @php
                            // Calculate high-level metrics for the user across all teams
                            $totalTasksCompleted = $user->teams()->withCount(['tasks' => function ($query) {
                                $query->where('status', 'done');
                            }])->get()->sum('tasks_count');
                        
                            $totalTeamsJoined = $user->teams()->count();
                        
                           $totalPoints = $user->teams()->sum('team_members.total_points');
                        @endphp
                        <div class="high-level-stats mt-3">
                            <p><strong>Total Tasks Completed:</strong> {{ $totalTasksCompleted }}</p>
                            <p><strong>Total Teams Joined:</strong> {{ $totalTeamsJoined }}</p>
                            ...
                            <a href="{{ route('profile.new_password_edit') }}" 
   class="btn mt-3 d-block"
   style="background-color: #00bcd4; color: white; border-radius: 8px; padding: 10px; font-weight: bold; transition: background-color 0.3s ease;">
