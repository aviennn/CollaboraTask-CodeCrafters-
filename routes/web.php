...
Route::middleware('auth')->group(function () {
    ...
    Route::get('/contact-us/create', [ContactUsController::class, 'create'])->name('contact.create'); // User form view
    Route::post('/contact-us', [ContactUsController::class, 'store'])->name('contact.store'); // Message submission
    Route::post('/contact-us/reply/{messageId}', [ContactUsController::class, 'reply'])->name('contact.reply'); // Admin reply
    Route::get('/contact/messages/{userId}', [ContactUsController::class, 'viewUserMessages'])->name('contact.viewUserMessages'); // View messages
    //Route::get('/messages/fetch', [ContactUsController::class, 'fetchMessages']);
});
...
//user-data-centric-report
Route::get('/admin/user-report', [ReportController::class, 'userReport'])->name('reports.userReport');
Route::get('/admin/user-report/data', [ReportController::class, 'getUserReportData'])->name('reports.userReportData');

// Define the route for filtering team tasks
Route::get('/tasks/filter-team', [TaskController::class, 'filterTeamTasks'])->name('user.tasks.filterTeamTasks');
//leave team route
Route::post('/teams/{team}/leave', [TeamController::class, 'leave'])->name('teams.leave');

Route::post('/tasks/{id}/resubmit', [TaskController::class, 'resubmitForApproval'])->name('user.tasks.resubmitForApproval');

//fetch active users count in admin dashboard
Route::get('/admin/fetch-active-users-count', [HomeController::class, 'fetchActiveUsersCount']);
Route::get('/admin/fetch-active-users-details', [HomeController::class, 'fetchActiveUsersDetails']);

Route::get('/teams/{team}/rewards-shop', [TeamController::class, 'showRewardShop'])->name('teams.rewards.shop');

Route::get('/activity-log/view/{user}', [UserController::class, 'viewActivityLog'])->name('activity-log.view');
...
Route::post('/tasks', [TaskController::class, 'store'])->name('user.tasks.store');
    ...
    // Task filtering route with a specific path for filtering by status
Route::get('/tasks/filter/{status?}', [TaskController::class, 'index'])
    ->where('status', 'not started|in progress|done|all')
    ->name('user.tasks.filter');
    ...
    return view('profile.partials.update-password-form');
             ...
             // routes/web.php
Route::get('/profile/new-update-password', [ProfileController::class, 'showNewUpdatePasswordForm'])->name('profile.new_password_edit');
Route::put('/profile/new-update-password', [ProfileController::class, 'newUpdatePassword'])->name('profile.new_password_update');
