...
{
    // Allow admins to access all team messages
    if (Auth::user()->usertype !== 'admin' && !Auth::user()->teams->contains($team->id)) {
        \Log::info('User not authorized', ['user_id' => Auth::id(), 'team_id' => $team->id]);
        return response()->json(['error' => 'Unauthorized'], 403);
    }

    \Log::info('User authorized', ['user_id' => Auth::id(), 'team_id' => $team->id]);
    $messages = $team->messages()->with('user')->get();
