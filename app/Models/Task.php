   ...
   public static function sendDueDateReminder()
{
    ...
    if ($tasksDueSoon->isEmpty()) {
        Log::info("No tasks found with upcoming due dates.");
    } else {
        foreach ($tasksDueSoon as $task) {
            ...
            if (in_array($daysRemaining, [1, 3, 7])) {
                Log::info("Task Found: " . $task->name . " is due in " . $daysRemaining . " days.");
                
                // Check if the user relationship is defined
                if ($task->user) {
                    $task->user->notify(new TaskDueReminderNotification($task, $daysRemaining));
                    Log::info("Notification sent to user: " . $task->user->name);
                } else {
                    Log::warning("No user assigned to task: " . $task->name);
                }
            }
        }
    }
    ...
    if ($overdueTasks->isEmpty()) {
        ...
        foreach ($overdueTasks as $task) {
            ...
            if ($task->assignee) {
                ...
                Log::warning("No assignee for overdue task: " . $task->name);
            }
        }
    }
}
